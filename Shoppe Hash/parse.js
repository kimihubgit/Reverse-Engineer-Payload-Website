// parse.js - Parse escaped string array thành list events {x, y, timestamp}

// Định nghĩa hàm unescape và parse
...
// Hàm parse chính
function parseEscapedArray(inputArray) {
    // Ghép tất cả chuỗi trong mảng và unescape
    const escapedStr = inputArray.join('');
    const unescapedStr = unescapeString(escapedStr);

    // Chuyển unescaped thành Uint8Array
    const bytes = new Uint8Array(unescapedStr.length);
    for (let i = 0; i < unescapedStr.length; i++) {
        bytes[i] = unescapedStr.charCodeAt(i) & 0xff; // Giữ byte 0-255
    }
    ...
    for (let i = 0; i + 8 <= bytes.length; i += 8) {
        ...
        const x = dv.getUint16(0, false); // big-endian
...
// Dữ liệu đầu vào: Thay bằng mảng của bạn ở đây
const inputArray = [
...
console.log(parseEscapedArray(inputArray));
